...
| 接口类型 | 路径                         | 说明          |
|------|----------------------------|-------------|
| POST | /api/questions/explanation | 上传问题，返回解释   |
| POST | /api/questions/keywords    | 搜索问题，返回关键词  |
| POST | /api/questions/relatenote  | 保存问题，返回相关笔记 |
| POST | /api/questions/autograde   | 保存问题，返回批改结果 |
| POST | /api/questions/record      | 保存问题，返回保存结果 |
### 前端请求接口
    ...
    "keywords": [
      {
        "term": "微分方程",
        "weight": 0.92,
        "related_questions": ["q_001", "q_003"]
      },
      {
        "term": "二阶导数",
        "weight": 0.85,
        "related_notes": ["note_2023"]
      }
    ]
  },
  "timestamp": 1719900000
...
```
- POST /api/questions/relatenote
### 前端请求接口
```typescript
interface RelateNoteParams {
  image: File;      // 问题图片文件
}
```
### 后端响应接口
...
```json
{
  ...
  "data": {
    ...
    "correct_rate": 0.85,
    "error_analysis": {
      "concept_errors": ["导数计算错误"],
      "step_errors": ["步骤跳步"]
    },
    "improvement_suggestions": ["建议加强导数运算练习"]
  },
  "timestamp": 1719900000
}
```
